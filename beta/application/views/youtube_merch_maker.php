<!DOCTYPE html>
<html lang="en-US">
    <head>
        <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=5.0, user-scalable=yes, minimal-ui">
        <meta charset="utf-8">
        <title>Merchandise maker for Youtubers | Eazyprint</title>
        <meta name="description" content="">
        <meta name="title" content="Merchandise maker for Youtubers | Eazyprint">
        
		<?php $this->load->view("common/metalinks");?>
    </head>
    <body class="sprd-responsive-cnt" style="">
	<?php $this->load->view("common/header");?>
        <main>
            

<div id="yt-merchandise">
    <section class="teaser">
        <div class="teaser__background"></div>
        <div class="container">
            <div class="teaser__wrapper">
                <h1 class="h0">YouTube Merchandise Maker</h1>
                <h3 class="h4">Create and sell merchandise for your YouTube community in your own free Shop</h3>
                <div class="sprd-button secondary" data-lightbox="register-modal">Open a FREE Eazyprint</div>
            </div>
        </div>
    </section>

    <section class="subnav">
        <div class="subnav">
            <div class="scroll-wrapper">
                <div class="subnav__wrapper">
                    <ul class="subnav__tabs">
                        <li class="subnav__tab" data-tribe="howTo" style="white-space: nowrap">How to Make Merchandise <span class="hideit">for Your YouTube Channel</span></li>
                        <li class="subnav__tab" data-tribe="products">Product Range</li>
                        <li class="subnav__tab" data-tribe="promote">Promote Your Merch</li>
                        <li class="subnav__tab" data-tribe="service">Design Assistance</li>
                        <li class="subnav__tab" data-tribe="shipping">Customer Support</li>
                    </ul>
                </div>
            </div>
        </div>
    </section>

    <section class="howTo">
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-12 col-lg-10">
                    <h2 class="h1 text-center">How to Make Merchandise for Your YouTube Channel</h2>
                </div>
            </div>
            <div class="row howTo__item">
                <div class="howTo__image">
                    <img src="<?php echo base_url('images/')?>OpenShop" alt="OOpen Shop">
                </div>
                <div class="howTo__text">
                    <h3>Open your own Shop</h3>
                    <p>If you're wondering how to make merchandise for your YouTube channel, then you're in the right place! Opening a Eazyprint is simple. Sign-up takes a matter of minutes. Before you know it, your shop will be up and running, just waiting for your wonderful designs!</p>
                </div>
            </div>

            <div class="row howTo__item">
                <div class="howTo__image">
                    <img src="<?php echo base_url('images/')?>UploadDesigns" alt="Upload Designs">
                </div>
                <div class="howTo__text">
                    <h3>Upload your designs</h3>
                    <p>Think of Eazyprint as your own personal YouTube merchandise maker. Whether you have one design or one hundred designs, everything can be easily uploaded onto any of our merchandise. From apparel to drinkware and accessories, you'll never run out of ideas for your Eazyprint!</p>
                </div>
            </div>

            <div class="row howTo__item">
                <div class="howTo__image">
                    <img src="<?php echo base_url('images/')?>LinkYourYT" alt="Link Youtube">
                </div>
                <div class="howTo__text">
                    <h3>Link your YouTube channel with your Shop</h3>
                    <p>One of the greatest benefits of Eazyprint is that you can easily link to your social media accounts from your shop's header. Utilizing your header image will allow you to drive traffic to your YouTube channel, to your website, to wherever you'd like! So, if you're looking to create merchandise for YouTubers, Eazyprint is well-equipped with all of the tools you'll need to succeed.</p>
                </div>
            </div>

            <div class="row howTo__item">
                <div class="howTo__image">
                    <img src="<?php echo base_url('images/')?>LinkYourShop" alt="Share Merch">
                </div>
                <div class="howTo__text">
                    <h3>Share your merch shop with your fans!</h3>
                    <p>Now it's time to spread the word about your incredible merchandise! Promote your Eazyprint with YouTube videos and social media posts. Add your shop link to your YouTube header so your Eazyprint is just one click away for all your dedicated subscribers!</p>
                </div>
            </div>
        </div>
    </section>
	<section class="four product-range products">
        <div class="container">
            <h2 class="h1 text-center">Product Range</h2>

            <div class="row justify-content-center">
                <div class="col-6 col-lg-2 product-range__tile" style="text-align:center;">
                    <img src="<?php echo base_url('images/')?>products_tshirt" alt="Premium T-shirts">
                    <p><b>Premium T-Shirts</b></p>
                </div>
				 <div class="col-6 col-lg-2 product-range__tile" style="text-align:center;">
                    <img src="<?php echo base_url('images/')?>products_hoodie" alt="Premium Hoodies">
                    <p><b>Premium Hoodies</b></p>
                </div>
                <div class="col-6 col-lg-2 product-range__tile" style="text-align:center;">
                    <img src="<?php echo base_url('images/')?>cap.jpg" alt="Premium Caps">
                    <p><b>Premium Caps</b></p>
                </div>
                <div class="col-6 col-lg-2 product-range__tile" style="text-align:center;">
                    <img src="<?php echo base_url('images/')?>jeans.jpg" alt="Premium Jeans">
                    <p><b>Premium Jeans</b></p>
                </div>
				<div class="col-6 col-lg-2 product-range__tile" style="text-align:center;">
                    <img src="<?php echo base_url('images/')?>products_mug" alt="Color Mugs">
                    <p><b>Color Mugs</b></p>
                </div>
				
            </div>

            <div class="row justify-content-center">
				<div class="col-6 col-lg-2 product-range__tile" style="text-align:center;">
					<img src="<?php echo base_url('images/')?>products_mobile" alt="Phone Cases">
					<p><b>Phone Cases</b></p>
				</div>
				 <div class="col-6 col-lg-2 product-range__tile" style="text-align:center;">
						<img src="<?php echo base_url('images/')?>tiles.jpg" alt="Floor Tiles">
						<p><b>Floor Tiles</b></p>
				  </div>
				<div class="col-6 col-lg-2 product-range__tile" style="text-align:center;">
					<img src="<?php echo base_url('images/')?>dinner_plates.jpg" alt="Dinner Plates">
					<p><b>Dinner Plates</b></p>
				</div>
				<div class="col-6 col-lg-2 product-range__tile" style="text-align:center;">
					<img src="<?php echo base_url('images/')?>cusions.jpg" alt="Pillows & Cusions" style="height:170px;">
					<p><b>Pillows & Cusions</b></p>
				</div>
				<div class="col-6 col-lg-2 product-range__tile" style="text-align:center;">
					<img src="<?php echo base_url('images/')?>rings.jpg" alt="Key Rings">
					<p><b>Key Rings</b></p>
				</div>
            </div>
            <div class="row">
                <div class="col-12">
                    <div class="products__button text-center">
                        <a class="sprd-button ghost ghost--dark" href="#">See All Products</a>
                    </div>
                </div>
            </div>
        </div>
    </section>


    <section class="promote">
        <div class="container">
            <h2 class="h1 text-center">Promote Your Merch</h2>
            <div class="row">
                <div class="col-6 col-lg-3 promote__item text-center">
                    <a href="https://www.spreadshirt.com/blog/tag/shop-marketing/">
                    <div class="promote__image">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
                            <g>
                                <path class="promote__icon" d="M2.16,2V12a4.19,4.19,0,0,0,2,3.55V26.89a3,3,0,0,0,3,3H24.76a3,3,0,0,0,3-3V15.5A4.19,4.19,0,0,0,29.66,12V2ZM23.28,4h4.38v8a2.19,2.19,0,0,1-4.38,0ZM16.91,4h4.37v8a2.19,2.19,0,1,1-4.37,0ZM10.53,4h4.38v8a2.19,2.19,0,1,1-4.38,0ZM4.16,4H8.53v8a2.19,2.19,0,1,1-4.37,0ZM25.72,26.89a1,1,0,0,1-1,1H7.11a1,1,0,0,1-1-1V16.15l.2,0a4.16,4.16,0,0,0,3.19-1.5,4.14,4.14,0,0,0,6.38,0,4.13,4.13,0,0,0,6.37,0,4.17,4.17,0,0,0,3.19,1.5,1.9,1.9,0,0,0,.25,0Z"></path>
                                <path class="promote__icon" d="M8.1,25.59H23.77v-7H8.1Zm2-5H21.77v3H10.1Z"></path>
                            </g>
                        </svg>
                    </div>
                    <div class="promote__text">Spreadshop Success Guide</div>
                    </a>
                </div>
                <div class="col-6 col-lg-3 promote__item text-center">
                    <a href="https://www.spreadshirt.com/blog">
                    <div class="promote__image">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
                            <g>
                                <path class="promote__icon" d="M29.5,11.1,16.8,5A1.1,1.1,0,0,0,16,5L2.5,11.1a1,1,0,0,0,0,1.8l5.4,2.7v9a2.27,2.27,0,0,0-1.2,2,2.2,2.2,0,1,0,4.4,0,2.27,2.27,0,0,0-1.2-2v-8l6,3a.6.6,0,0,0,.4.1.9.9,0,0,0,.5-.1l4.3-2.2.9,4.2c-.8.8-3.1,2.7-7.8,2.3a1,1,0,0,0-1.1.9,1.17,1.17,0,0,0,.9,1.1c.5,0,1,.1,1.5.1,5.8,0,8.3-3.3,8.4-3.5l.3-.4L23,16.4l6.6-3.5a1.05,1.05,0,0,0,.5-.9A1,1,0,0,0,29.5,11.1ZM16.3,17.6,11.1,15l5.4-2.7a.88.88,0,0,0,.4-1.3.88.88,0,0,0-1.3-.4L8.9,14,5.3,12.2l11.1-5,10.5,5Z"></path>
                            </g>
                        </svg>
                    </div>
                    <div class="promote__text">Tips, Tricks and Updates from Our Blog</div>
                    </a>
                </div>
                <div class="col-6 col-lg-3 promote__item text-center">
                    <a href="https://www.spreadshirt.com/blog/2016/11/21/placeit-customizable-advertising-material-for-your-shop/">
                    <div class="promote__image">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
                            <g>
                                <path class="promote__icon" d="M27.38,8.59H22.06l-.6-1C21,6.81,20.23,5.5,18.71,5.5H13.28c-1.56,0-2.36,1.42-2.74,2.08l-.61,1H4.59A2.61,2.61,0,0,0,2,11.19V23.9a2.6,2.6,0,0,0,2.58,2.6H27.39A2.59,2.59,0,0,0,30,23.91h0V11.19A2.61,2.61,0,0,0,27.38,8.59Zm0,15.91H4.58a.6.6,0,0,1-.6-.6V11.19a.6.6,0,0,1,.6-.6H10.5a1,1,0,0,0,.86-.48l.92-1.52c.44-.78.73-1.09,1-1.09H18.7c.31,0,.63.42,1,1.11l.9,1.49a1,1,0,0,0,.86.48h5.88a.6.6,0,0,1,.6.6L28,23.9A.6.6,0,0,1,27.4,24.5ZM16,11.66a5.8,5.8,0,1,0,5.8,5.8A5.8,5.8,0,0,0,16,11.66Zm0,9.59a3.8,3.8,0,1,1,3.8-3.8A3.8,3.8,0,0,1,16,21.25Z"></path>
                            </g>
                        </svg>
                    </div>
                    <div class="promote__text">Professional Images with Placeit</div>
                    </a>
                </div>
                <div class="col-6 col-lg-3 promote__item text-center">
                    <a href="https://www.spreadshirt.com/blog/2014/08/13/google-adwords-tips-for-partners/">
                        <div class="promote__image">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
                                <g>
                                    <polygon class="promote__icon" points="8.94 23.02 2.02 16 8.94 8.98 10.36 10.4 4.85 16 10.36 21.6 8.94 23.02"></polygon>
                                    <polygon class="promote__icon" points="23.1 8.98 30.02 16 23.1 23.02 21.69 21.6 27.19 16 21.69 10.4 23.1 8.98"></polygon>
                                    <polygon class="promote__icon" points="14.05 25.97 12.12 25.45 18.01 6.03 19.94 6.55 14.05 25.97"></polygon>
                                </g>
                            </svg>
                        </div>
                        <div class="promote__text">Google Adwords Integration</div>
                    </a>
                </div>
            </div>
        </div>
    </section>

    <section class="service">
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-lg-11 service__item">
                    <div class="service__head">
                        <h2 class="h1">Design Assistance</h2>
                    </div>

                    <div class="service__image h1">
                        <img src="<?php echo base_url('images/')?>design-services.gif" alt="Design Service">
                    </div>

                    <div class="service__text">
                        <p>Need your own custom design? We've got your back! Our premium design team will put their creativity to work, so you'll have AMAZING designs that will WOW your fans!</p>
                        <a class="sprd-button ghost ghost--dark" href="#">
                            Get Your Own Design
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </section>

  
	<section class="endTeaser">
        <div class="container">
            <div class="row text-center">
                <div class="col-12">
                    <h2 class="h1">Ready to Start Selling Your Merch?</h2>
                    <p>No matter your passion, we'll make it happen.</p>
                    <div class="sprd-button secondary" data-lightbox="register-modal">Open a FREE Eazyprint
					</div>
                </div>
            </div>
        </div>
    </section>
    <section class="shipping">
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-12 col-lg-10">
                    <h2 class="h1">Shipping & Customer Support</h2>
                </div>
                <div class="col-12 col-lg-5">
                    <p>When it comes to merchandise for YouTubers, it's important to make things as easy as possible. You're busy building a brand, so we'll take care of production, fulfillment and shipping at no cost to you!</p>
                    <p>If you have questions, our award-winning customer support has answers. With 15+ years of industry experience, we've seen and heard it all, so ask away!</p>
                </div>

                <div class="col-12 col-lg-5">
                    <p>Additionally, Eazyprint offers a strong community on our forum and on our social media profiles. By utilizing these resources, you can connect with like-minded people and discuss your ideas, questions and goals as a shop owner.</p>
                    <p>We want to support you as much as possible, so we have nothing but resources ready at your convenience. You'll always be in-the-know about your Eazyprint, so you can keep on selling!</p>
                </div>
            </div>
        </div>
    </section>


</div>        </main>

    <?php $this->load->view("common/footer");?>
    <script src="<?php echo base_url('js/script_merch_maker.js');?>"></script>    
</body>
</html>