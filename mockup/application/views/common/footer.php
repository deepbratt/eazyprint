	<style>
	.fmnu li a:hover{
		text-decoration:underline;
	}
	</style>

	

	<!--<div class="container">
		<div class="row">
			<p style="text-align:center;"><img src="<?php echo base_url();?>images/Selling-Scrap-Online.png" style="text-align:center;"></p>
		</div>
	</div>-->
	
	
	<div class="container-fluid" style="background:#ffe4ca;margin-bottom:-50px;">
		<div class="container" >
			<p>&nbsp;</p>

			<div class="row">
				<div class="col-md-10">
					<h4 style="text-align:left;color:#000;font-family:Arial;margin-top:20px;font-size:40px;">Need large volume for your organisation or for some special occasion/event ?</h4>
				</div>
				<div class="col-md-2">
					<button class="btn btn-lg btn-danger" style="margin-top:30px;font-size:25px;"> GET A QUOTE </button>
				</div>
			</div>
			
			<p>&nbsp;</p>
		</div>
	</div>
	<!--<div class="container" style="padding-top:3px;">
		<div class="row">
			<div class="col-md-4 col-sm-12"  style="padding:50px;">
				<div class="card text-white bg-danger">
					<div class="card-body">
						<p class="card-text" style="font-size:25px;color:#fff;"><i class="fas fa-truck" style="font-size:50px;color:#fff;"></i> &nbsp; FREE Delivery</p>
					</div>
				</div>
			</div>
			
			<div class="col-md-4 col-sm-12" style="padding:50px;">
				<div class="card text-white bg-danger">
					<div class="card-body">
						<p class="card-text"style="font-size:25px;color:#fff;"><i class="fas fa-rupee-sign" style="font-size:50px;color:#fff;"></i> &nbsp;Cash On Delivery</p>
					</div>
				</div>
			</div>

			<div class="col-md-4 col-sm-12" style="padding:50px;">
				<div class="card text-white bg-danger">
					<div class="card-body">
						<p class="card-text"style="font-size:25px;color:#fff;"><i class="fas fa-undo" style="font-size:50px;color:#fff;"></i> &nbsp;Easy returns</p>
					</div>
				</div>
			</div>
		</div>
		<p>&nbsp;</p>
	</div>-->

<!--footer-->
				<p style="border-top:1px solid #CCC:">&nbsp;</p>
				<footer class="footer" style="background:#eceef9;border-top:1px solid #c3c5d4 !important;">
			
					
					<div class="container" >
					    <!-- footer top -->
						<div class="container">
						  <div class="row" style="margin-bottom:20px;">
								<div class="col-md-4"></div>
								<div class="col-md-4" style="text-align:center;">
										<a href="#"><img src="<?php echo base_url();?>images/logo.png" style="background:#fff;height:70px;background:#fff;border:1px solid #CCC;margin-top:-70px;border-radius:15px;"></a>
								</div>
								<div class="col-md-4"></div>
							</div>


						  <div class="row">

								<div class="col-6 col-md-3 col-xs-6">
								  <div class="footerlinks">
									<h5 class="fred">NEED HELP
									</h5> 
									<ul class="fmnu">

									  <li>
										<a href="/contact-us" class="router-link-exact-active router-link-active" style="color:#000;text-transform:uppercase;">Contact Us
										</a>
									  </li>
									  
									  <li>
										<a href="/faq/myorder" class="" style="color:#000;text-transform:uppercase;">FAQs
										</a>
									  </li>

									  <li>
										<a href="/faq/returnexchange" class="" style="color:#000;text-transform:uppercase;"> Offers
										</a>
									  </li>

									  <li>
										<a href="/terms-and-conditions" class="" style="color:#000;text-transform:uppercase;">Refund Policy
										</a>
									  </li>

									  <li>
										<a href="/terms-and-conditions" class="" style="color:#000;text-transform:uppercase;">Track Order
										</a>
									  </li>

									</ul>
								  </div>
								</div> 

								<div class="col-6 col-md-3 col-xs-6">
								  <div class="footerlinks">
									<h5 class="fred">CATEGORIES
									</h5> 
									<ul class="fmnu">
									  <?php
											$ci =&get_instance();
											$ci->load->model('home_m');
											$get_categories = $ci->home_m->get_cat();				
											foreach($get_categories AS $get_cat){
									  ?>
									  <li>
										<a href="<?php echo base_url();?>link/<?php echo $get_cat->cat_id;?>" class="" style="color:#000;text-transform:uppercase;"> <?php echo $get_cat->category_name;?>
										</a>
									  </li> 
									  <?php
											}
									  ?>

									</ul>
								  </div>
								</div> 

								<div class="col-6 col-md-3 col-xs-6 showdesktop">
								  <div class="footerlinks">
									<h5 class="fred"> COLLABORATION
									</h5> 
									<ul class="fmnu">
									  <li>
										<a href="/profile" class="" style="color:#000;text-transform:uppercase;"> Bulk Orders
										</a>
									  </li> 
									  <li>
										<a href="/gift-voucher" class="" style="color:#000;text-transform:uppercase;"> Merchandise Solution
										</a>
									  </li> 
									  <li>
										<a href="/souled-army" class="" style="color:#000;text-transform:uppercase;"> Partner Stores
										</a>
									  </li> 
									  <!--<li>
										<a href="/submit-art-work" class="" style="color:#000;"> Affiliate
										</a>
									  </li>
									  <li>
										<a href="/submit-art-work" class="" style="color:#000;"> Dropshipping
										</a>
									  </li>-->
									  
									</ul>
								  </div>
								</div> 

								<div class="col-6 col-md-3 col-xs-6">
								  <div class="footerlinks notAnimate">
									<h5 class="fred">CONTACT US
									</h5>
									<ul>
									  <li class="showdesktop">
										<i class="fas fa-envelope" style="color:#000;"></i>  <span style="margin-left:5px;color:#000;">: sales@eazyprint.in </span>
									  </li>

									  <li class="showdesktop">
										<i class="fas fa-mobile-alt" style="color:#000;"></i> <span style="margin-left:7px;color:#000;">: +91 72788 42023 </span>
									  </li>

									  <li class="showdesktop">
										<i class="fas fa-mobile-alt" style="color:#000;"></i> <span style="margin-left:7px;color:#000;">: +91 92308 41054 </span>
									  </li>

									  <li class="showdesktop">
										<i class="fas fa-map-marked" style="color:#000;"></i> <span style="margin-left:5px;color:#000;">: Barrackpore , Kol-700122 , INDIA </span>
									  </li>

									</ul>
								  </div>
								</div> 

						  </div> 

					</div> 

						<!-- footer top ends -->
						<div class="row align-items-center flex-row-reverse" style="margin-top:10px;border-top:1px dotted #CCC;border-bottom:1px dotted #CCC;">
							<div class="col-lg-6 col-sm-12" style="color:#000;">
								100% Secure Payment: <img src="<?php echo base_url();?>images/masterdcard.png" style="height:24px;margin:5px;"> <img src="<?php echo base_url();?>images/paytm.png" style="height:24px;margin:5px"> <img src="<?php echo base_url();?>images/cod.png" style="height:24px;margin:5px"> <img src="<?php echo base_url();?>images/instamojo-logo-white_0.png" style="height:24px;margin:5px">
							</div>

							<div class="col-lg-6 col-sm-12" style="color:#000;">
								Shipping Partners: <img src="<?php echo base_url();?>images/fedex.png" style="height:30px;margin:5px;"> <img src="<?php echo base_url();?>images/dtdc.png" style="height:30px;margin:5px"> <img src="<?php echo base_url();?>images/delivery.png" style="height:30px;margin:5px">
							</div>
						</div>


						<div class="row align-items-center flex-row-reverse" style="margin-top:10px;">
							<div class="col-lg-12 col-sm-12 mt-3 mt-lg-0 text-center fred" style="color:#000;font-weight:bold;">
								Made with love in <u> INDIA </u>
							</div>
						</div>
					</div>
				</footer>
				<!-- End Footer-->
            </div>
		</div>
		<a href="#top" id="back-to-top" style="display: inline;"><i class="fas fa-angle-up"></i></a>
		<!-- Dashboard Core -->

		<!--<script src="<?php echo base_url();?>js/jquery-3.2.1.min.js"></script>-->
		<script src="<?php echo base_url();?>js/bootstrap.bundle.min.js"></script>
		<script src="<?php echo base_url();?>js/jquery.sparkline.min.js"></script>
		<script src="<?php echo base_url();?>js/selectize.min.js"></script>
		<script src="<?php echo base_url();?>js/jquery.tablesorter.min.js"></script>
		<script src="<?php echo base_url();?>js/circle-progress.min.js"></script>
		<script src="<?php echo base_url();?>js/jquery.rating-stars.js"></script>
		<script src="<?php echo base_url();?>js/sidemenu.js"></script>
		<script src="<?php echo base_url();?>js/jquery.mCustomScrollbar.concat.min.js"></script>
		<script src="<?php echo base_url();?>js/d3.v5.min.js"></script>
		<script src="<?php echo base_url();?>js/c3-chart.js"></script>
		<script src="<?php echo base_url();?>js/jquery.mask.min.js"></script>
        <script src="<?php echo base_url();?>js/circle-progress.min.js"></script>
       	<script src="<?php echo base_url();?>js/particles.js"></script>
        <script src="<?php echo base_url();?>js/particlesapp_default.js"></script>
		<script src="<?php echo base_url();?>js/jquery.vmap.js"></script>
		<script src="<?php echo base_url();?>js/jquery.vmap.min.js"></script>
		<script src="<?php echo base_url();?>js/jquery.vmap.sampledata.js"></script>
		<script src="<?php echo base_url();?>js/jquery.vmap.world.js"></script>
		<script src="<?php echo base_url();?>js/counterup.min.js"></script>
		<script src="<?php echo base_url();?>js/waypoints.min.js"></script>
		<script src="<?php echo base_url();?>js/index.js"></script>
		<script src="<?php echo base_url();?>js/charts.js"></script>
		<script src="<?php echo base_url();?>js/counterup.min.js"></script>
		<script src="<?php echo base_url();?>js/waypoints.min.js"></script>
		<script src="<?php echo base_url();?>js/Chart.bundle.js"></script>
		<script src="<?php echo base_url();?>js/WidgetChart.js"></script>
		<script src="<?php echo base_url();?>js/custom.js"></script>
		<script src="<?php echo base_url();?>js/slick.js"></script>

		<script>
			window.onscroll = function() {myFunction()};

			var header = document.getElementById("myHeader");
			var sticky = header.offsetTop;

			function myFunction() {
			  if (window.pageYOffset > sticky) {
			    header.classList.add("sticky");
			  } else {
			    header.classList.remove("sticky");
			  }
			}
		</script>
		

		<!-- ACCORDIONS STARTS -->
			<script>
				var acc = document.getElementsByClassName("accordion");
				var i;

				function click_action(){
				  $('.accordion').removeClass('active');
				  $('.panel').removeClass('show');

				  this.classList.toggle("active");
				  this.nextElementSibling.classList.toggle("show");
				}

				for (i = 0; i < acc.length; i++) {
				  acc[i].onclick = click_action;
				}
			</script>
		<!-- Accordions Ends -->