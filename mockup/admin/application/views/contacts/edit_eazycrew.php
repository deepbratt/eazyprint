...
<html lang="en" dir="ltr">
	<head>
		...
		<!-- Title -->
		<title>Eazyprint | Edit Eazycrew</title>
		<style>
		.error{
			color : red;
			font-weight: 450;
				...
				<div class="my-3 my-md-5 app-content">
					<div class="side-app">
						<div class="page-header">
							<h4 class="page-title">Edit Eazycrew &nbsp;&nbsp;<a href="<?php echo base_url("listing_eazycrew");?>" class="btn btn-primary">View All</a></h4>
							<ol class="breadcrumb">
								<li class="breadcrumb-item"><a href="#">Eazycrew</a></li>
								<li class="breadcrumb-item active" aria-current="page">Edit Eazycrew</li>
							</ol>
						</div>
						<?php
						...
						<div class="row">
							<div class="col-md-12">
								<form name="add_eazycrew" method="post" action="<?php echo base_url('edit_eazycrew/update_dealer');?>/<?php echo $this->uri->segment(2);?>">
								<div class="card">
											...
											<div class="col-md-6">
												<div class="form-group">
													...
													<select class="form-control" name="crew_role">
														...
														<option value="admin" <?php echo (($fetch_dealer_data->user_crew_role == 'admin')?'selected':'')?>> Admin </option>
														<option value="accounts" <?php echo (($fetch_dealer_data->user_crew_role == 'accounts')?'selected':'')?>> Accounts </option>
														<option value="operations" <?php echo (($fetch_dealer_data->user_crew_role == 'operations')?'selected':'')?>> Operations </option>
														<option value="marketing" <?php echo (($fetch_dealer_data->user_crew_role == 'marketing')?'selected':'')?>> Marketing </option>
													</select>
												</div>
											</div>
	...
	<script>
	$(function() {
	  $("form[name='add_eazycrew']").validate({
		rules: {
		  f_name: "required",
		  l_name: "required",
		  email : "required",
		...
		messages: {
		  ...
		  crew_role : "This field is required",
		},
	...
	<script>
		...
		function state_name(state){
				$.ajax({
				url: '<?php echo base_url();?>edit_eazycrew/ajax_state_name',
				data: {'state': state,},
				type: "post",
				beforeSend: function(){
			        $('#AjaxLoader').show();
